...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    match Ticket::new(title.clone(), description.clone(), status.clone()) {
        Ok(ticket) => ticket,
        Err(TicketNewError::WrongTitle ( description )) => panic!("{}", description.as_str()),
        Err(TicketNewError::WrongDescription ( _ )) => {
...
impl Ticket {
    pub fn new(
        ...
        if title.is_empty() {
            return Err(TicketNewError::WrongTitle("Title cannot be empty".to_string()));
        }
        if title.len() > 50 {
            return Err(TicketNewError::WrongTitle("Title cannot be longer than 50 bytes".to_string()));
        }
        if description.is_empty() {
            return Err(TicketNewError::WrongDescription("Description cannot be empty".to_string()));
        }
        if description.len() > 500 {
            return Err(TicketNewError::WrongDescription("Description cannot be longer than 500 bytes".to_string()));
        }
